...
class RLRecommender(Recommender):
    ...
    def _fit(
        ...
        env = FakeRecomenderEnv(val_df, self.k)
        evaluate_scorer = evaluate_on_environment(env)
	
        if self.fitter is None:
            
            self.fitter = self.model.fitter(self.train, n_epochs=self.n_epochs, eval_episodes=self.train, scorers={'environment': evaluate_scorer})
